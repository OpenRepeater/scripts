...
################################################################################
# PRE-INSTALL configuration start
################################################################################
...
(
	...
	########################################################
	# Configure Wifi Country Domain
	########################################################	
	set_wifi_domain
	####################################################
	# add serial consile to allow access where no 
	# network avaible. Raspi zero/w/w2/4b (New)
	####################################################
	otg_console
	########################################################
	...
	if [ "$INPUT_INSTALL_TYPE" = "ORP" ]; then
		########################################################
		# if installing ORP default We need to add some settings
		# to the config.txt file to enable ics interface cards 
		# or they won't load up properly.
		########################################################
		config_ics_controllers
	else
		########################################################
		# if installing Custom Build We need to add the new 
		# driver setup script to enable ics interface cards 
		# or they won't load up properly.
		########################################################
		install_ics_drivers
	fi
	########################################################
	# ICS tweaks to keep the audio channels seperated
	########################################################
	set_ics_asound
	####################################################
